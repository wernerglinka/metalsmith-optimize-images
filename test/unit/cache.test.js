...
function runBuild(fixturesDir, buildDir, options, clean, prePlugin) {
  return new Promise((resolve, reject) => {
    const ms = Metalsmith(fixturesDir).clean(clean).source('src').destination(buildDir);
    ...
    ms.use(optimizeImages(options)).build((err) => {
      ...
      const responsiveFiles = fs.existsSync(buildResponsiveDir) ? fs.readdirSync(buildResponsiveDir).sort() : [];
...
describe('Persistent source-tree cache', () => {
  ...
  it('should load variants from cache on subsequent builds (skip Sharp)', async () => {
    ...
    assert.deepStrictEqual(second.cacheFiles, first.cacheFiles, 'second build should have the same cached files');
  ...
  it('should regenerate variants when source image content changes (hash changes)', async () => {
    ...
    const differentImageBuffer = fs.readFileSync(path.join(fixturesDir, 'src', 'images', 'industry.jpg'));
    ...
    const newTreeVariants = second.cacheFiles.filter((f) => f.startsWith('tree-') && !f.includes(originalHash));
    assert.ok(
    ...
    assert.ok(!srcset.includes(originalHash), 'HTML should reference the new hash, not the original');
  });
  ...
  it('should not add variant files to the Metalsmith files object when cache is enabled', async () => {
    ...
    const variantKeys = Object.keys(filesSnapshot).filter((key) => key.startsWith(pluginOptions.outputDir));
    assert.strictEqual(
      variantKeys.length,
      0,

...
describe( 'Testbed Simulation Debug', function () {
  ...
  it( 'should simulate testbed structure and show debug info', ( done ) => {
    ...
    metalsmith.build( ( err, files ) => {
      ...
      const backgroundVariants = processedVariants.filter( variant => 
      ...
      const htmlVariants = processedVariants.filter( variant => 
        variant.includes( '2005.01.003' ) || 
        variant.includes( '2006.02.001' )
      );

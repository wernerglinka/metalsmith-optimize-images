...
describe( 'Background Image Processing Debug', function () {
  ...
  it( 'should debug background image processing logic', ( done ) => {
    ...
    metalsmith.build( ( err, files ) => {
      ...
      // Check for background image variants
      const backgroundVariants = generatedFiles.filter( file => 
        file.includes( 'background-test' ) && file.includes( 'assets/images/responsive/' )
      );

      const htmlImageVariants = generatedFiles.filter( file => 
        file.includes( 'tree-' ) && file.includes( 'assets/images/responsive/' )
      );

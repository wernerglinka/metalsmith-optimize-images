...
describe('Filesystem Background Image Processing', () => {
  ...
  it('should find and process images from filesystem that are not in Metalsmith files', (_t, done) => {
    const metalsmith = Metalsmith(sourceDir).clean(true).destination(buildDir);
    ...
    metalsmith.build((err, files) => {
      ...
      const headerVariants = generatedFiles.filter(
        (file) => file.includes('header') && file.includes('assets/images/responsive/')
      );
